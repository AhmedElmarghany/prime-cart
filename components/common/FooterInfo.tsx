import { RiTimeLine as Clock, RiMailSendLine as Mail, RiMapPin2Line as MapPin, RiPhoneLine as Phone } from "@remixicon/react";
import React from "react";

interface ContactItemData {
  title: string;
  subtitle: string;
  icon: React.ReactNode;
}

const data: ContactItemData[] = [
  {
    title: "Visit Us",
    subtitle: "New Cairo, EG",
    icon: (
      <MapPin className="h-6 w-6 text-foreground group-hover:text-primary transition-colors" />
    ),
  },
  {
    title: "Call Us",
    subtitle: "+20 109 54 96 727",
    icon: (
      <Phone className="h-6 w-6 text-foreground group-hover:text-primary transition-colors" />
    ),
  },
  {
    title: "Working Hours",
    subtitle: "Mon - Sat: 10:00 AM - 7:00 PM",
    icon: (
      <Clock className="h-6 w-6 text-foreground group-hover:text-primary transition-colors" />
    ),
  },
  {
    title: "Email Us",
    subtitle: "ahmedelmarghany01@gmail.com",
    icon: (
      <Mail className="h-6 w-6 text-foreground group-hover:text-primary transition-colors" />
    ),
  },
];

const FooterInfo = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 border-b">
      {data?.map((item, index) => (
        <div
          key={index}
          className="flex items-center gap-3 group hover:bg-popover p-4 transition-colors hoverEffect"
        >
          {item?.icon}
          <div>
            <h3 className="text-foreground font-semibold group-hover:text-popover-foreground hoverEffect">
              {item?.title}
            </h3>
            <p className="text-secondary-foreground text-sm mt-1 group-hover:text-popover-foreground hoverEffect">
              {item?.subtitle}
            </p>
          </div>
        </div>
      ))}
    </div>
  );
};

export default FooterInfo;
